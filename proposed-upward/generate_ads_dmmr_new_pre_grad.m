...
%             ori_stego_dir = sprintf('%s-dmmr-payload-%.2f-use-dcts-13', cover_dir, payload); 
%             stego_dir = sprintf('%s-dmmr-new-payload-%.2f-use-dcts-13', cover_dir, payload); 
...
%     attack_QF = QF;
    ...
    cover_dir = sprintf('%s/%s-cover-resample-%d-jpeg-%d',base_dir, image_set, image_size, QF); 
    afterchannel_stego_dir = sprintf('%s/DMMR/re_stego', base_dir); 
%     ori_stego_dir = sprintf('%s-dmmr-upward-payload-%.2f-use-dcts-13', cover_dir, payload); 
    ori_stego_dir = sprintf('%s-dmmr-upward-payload-%.2f-use-dcts-13', cover_dir, payload);
...
%     bit_error_rate = zeros(1,cover_num); 
    bit_error_rate = zeros(11,attack_times,cover_num); 
...
%%  ��ϢǶ��    
    parfor i_img = 1:cover_num
        ...
        cost_path = fullfile([cost_dir,'/',index_str,'.mat']);
        ...
        msg_path = fullfile([msg_dir,'/',index_str,'.mat']);
...
%         flat_cover_grad = reshape(temp_cover_grad,1,image_size*image_size);
        ...
        flat_cover_grad = cover_grad(~rep_emb_fre);
...
%             myp1 = myp1s(p1_index);
            ...
            % greater p grad
%             x_grad_robust = x_grad(~rep_emb_fre);
            high_cover_grad = x_grad(max(1,round(length(x_grad)*myp1)));
...
%             ae_cover_mod_locations = logical(~rep_emb_fre .* abs_cover_grad .* abs(Modifications) .* grad_same);
            ...
            ae_cover_mod_locations = logical(~rep_emb_fre .* abs_cover_grad .* abs(Modifications));
...
%             recompression(ae1_stego_path,QF,ae1_stego_recom_path,C_QUANT,QF,code)
    ...
    %             imwrite(imread(stego_path),afterchannel_stego_path,'quality',Facebook_attack_QF);  
        ...
        %%  ����ÿ��ͼ���������??      
                ...
                bit_error_rate(5,a_t,i_img) = bit_error_number;
            end
...
%         
    ...
    ave_error_rates = mean(bit_error_rate, 3);
    save('test_p1_ber_grad_cover.mat', 'ave_error_rates');
    file_id = fopen('ddmr_adv_log_ber_new_upward.txt', 'a');
    for p1_index = 5
        myp1 = myp1s(p1_index);
        for p_t = 1:attack_times
            attack_QF = attack_QFs(p_t);
            ave_error_rate = ave_error_rates(p1_index,p_t);

            fprintf(file_id,'image_size-%d-QF-%d-%s-%s\n',image_size, attack_QF,num2str(myp1), ['payload: ',num2str(payload),'  ave_error_rate: ',num2str(ave_error_rate)]);  
        end
    end
    fclose(file_id);
