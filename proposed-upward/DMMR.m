function [cover_lsb,change,rho] = DMMR(use_dcts,cover_Path,rho1_P,rho1_M,tab_m)
%% 抖动调制机制计算载体元素的修改失真和修改距离

[min_value, max_value,~] = get_sum(use_dcts);
...
for bm = 1:m_block
    for bn = 1:n_block
        for i = 1:8
            for j = 1:8
                if (i+j>=min_value) && (i+j<=max_value)%中频9个DCT系数
                    n_lsb = n_lsb + 1;
