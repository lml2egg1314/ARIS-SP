...
% for index = 5818 : 5818
  ...
  [~, COEFFS] = dct2spatial(c_struct);

%   img = output_image;


  save_dct(output_jpeg_path, COEFFS);

end
end

function save_dct(output_path, COEFFS)

save(output_path, 'COEFFS', '-v6');
end
function save_nr(output_path, img)
