...
class MyDataset(Dataset):
	...
	def __init__(self, cover_dir, stego_dir, index_path, transform=None):
		...
		self.cover_path = cover_dir + '/{}.mat'
		self.all_stego_path = stego_dir + '/{}.mat'
...
def trainNet(output_dir, cover_dir, stego_dir, iteration, gpu_num, train_index_path, test_index_path):
  ...
  test_dataset = MyDataset(cover_dir, stego_dir, index_path=test_index_path, transform=eval_transform)
  ...
  # 			best_acc = evaluate(model, device, valid_loader,best_acc, epoch, optimizer, pt_path)
  ...
  # 	adjust_bn_stats(model, device, train_loader)
  # 	acc = evaluate(model, device, valid_loader, best_acc, epoch, optimizer, pt_path)
  ...
  for epoch in range(startEpoch, EPOCHS + 1):
      ...
      if epoch % EVAL_PRINT_FREQUENCY == 0:
        adjust_bn_stats(model, device, train_loader)
        acc = evaluate(model, device, test_loader, epoch, optimizer, pt_path)
