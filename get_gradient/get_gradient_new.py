...
class MyDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		cover_data = sio.loadmat(cover_path)['COEFFS']
		stego_data = sio.loadmat(stego_path)['COEFFS']
		data = np.stack([cover_data, stego_data])
		label = np.array([0,0], dtype='int32')
...
def calSignGrad(pt_path, indexPath, cover_dir, stego_dir, grad_dir, gpu_num, quality):
	...
	for i, sample in enumerate(data_loader):
		...
		cover_grad = grad.cpu().numpy().squeeze()
		pred =pred.cpu().numpy()
